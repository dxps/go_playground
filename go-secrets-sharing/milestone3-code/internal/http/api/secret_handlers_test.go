...
func Test_getSecretHandler(t *testing.T) {
	...
	r := httptest.NewRequest(http.MethodPost, "/secrets", body)
	...
	testcases := []TestCase{
		{
			...
			requestMethod:      http.MethodPost,
			requestURLPath:     "/secrets/{not-relevant}",
			expectResponseCode: http.StatusMethodNotAllowed,
		...
		},
		{
			...
			requestMethod:      http.MethodGet,
			requestURLPath:     "/secrets/57c3c3e8874431efd7ae79a8972bdfd4", // ID corresponding to "snap1" secret.
			expectResponseCode: http.StatusOK,
